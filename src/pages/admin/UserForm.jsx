...
const UserForm = ({ user, onSubmitSuccess, onCancel, isAdmin = false }) => {
  ...
  const handleAdminPasswordChange = async (e) => {
    ...
    try {
      ...
      // Más información: status, data y message
      const status = err?.response?.status;
      const data = err?.response?.data;
      const message = data?.error || data?.message || err?.message || "Error actualizando la contraseña.";
      setPwdError(`(${status || 'network'}) ${message}`);
    } finally {
      setPwdLoading(false);
    }
  };
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-4">
          ...
          <select
            ...
            onChange={handleChange}
            className={`block w-full px-3 py-2 border ${errors.roleId ? "border-red-500" : "border-gray-300"
